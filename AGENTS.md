...
- **US-16 — Upload vault to Google Drive**: Not started. `drive.file` OAuth scope added at login; `internal/sync` package with `DriveClient`, `State` (Pebble-backed), `Watcher` (fsnotify + 2 s debounce), `SyncManager` (upload path + initial reconciliation); config `DriveSync` struct.
