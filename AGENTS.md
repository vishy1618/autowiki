...
- **US-18 — Drive sync status**: Not started. `GET /api/drive/status` endpoint (`enabled`, `connected`, `last_vault_sync`, `last_pebble_backup`, `last_error`); UI status indicator with relative timestamp and error state.
