...
- **US-13 — Sliding session expiry**: Not started. Token rotation in auth middleware (rotate when <15 min remaining), 30-day absolute lifetime, 30-second grace window for in-flight requests.
- **US-14 — Search past conversations**: Not started. `SearchMessages` on `ChatStore`, `search_chat_history` LLM tool (3 sessions/call, max offset 50), chat handler dispatch.
