...
- **US-11 — Overnight wiki consolidation**: Complete. `vault.Manager` extended with `safePath`, `ListVault`, `ReadFilePartial`, `MoveFile`, `DeleteItem`; four new LLM tool definitions (`list_vault`, `read_page_partial`, `move_page`, `delete_item`) plus safety rule in system prompt; chat handler dispatches all seven tools with `vault` SSE events for mutations; `dream.Runner` schedules nightly runs within a configurable UTC window (default 19–23 UTC ≈ 1–5 am IST, set via `start_hour_utc`/`end_hour_utc` in config) with idempotency via `log.md` date check; `dream.Consolidate(ctx, vm, streamer)` runs the full agentic loop via `chat.AgenticRunner` + `MemChatStore` (max 50 tool calls), appends `dream started` / `dream ended - <20-word summary>` to `log.md`; `POST /api/dream/run` triggers a manual run (202 response).
- **US-12 — Check dream state history**: Complete. Implemented as part of US-11 — dream appends `dream started` and `dream ended - <summary>` (with RFC3339 timestamp prefix) to `log.md` after each run; model reads it via `read_page` on user request.
...
- **US-16 — Upload vault to Google Drive**: Complete. `drive.file` OAuth scope + offline access added to sign-in flow; refresh token persisted via `DriveTokenStore`; `internal/drivesync` package: `DriveClient` (EnsureFolder, EnsureFolderPath, Upload, Trash, ListFolder, GetStartPageToken), `State` (Pebble-backed sync metadata), `Watcher` (fsnotify, 2 s debounce per path, recursive, synthetic events for new dirs), `SyncManager` (initial reconcile on startup, live watcher consumer, single upload worker, graceful shutdown). Page token preserved across restarts. `config.yaml` has `drive_sync` block.
- **US-17 — Sync Drive changes back to vault**: Complete. Polling goroutine (`changes.list`, 60 s, single retry on transient error); download path with echo suppression via `pendingDownloads`; full bidirectional `Reconcile` (upload-walk + `ListFolder` download pass); conflict resolution (`last_write_wins` default / `keep_both` with 5 s window, pure `Resolve` function in `conflict.go`); Drive deletions propagate locally; `DriveModTime` stored after download to prevent re-download loops; all config fields exposed as env vars (`DRIVE_SYNC_ENABLED`, `DRIVE_SYNC_ROOT_FOLDER`, `DRIVE_SYNC_VAULT_FOLDER`, `DRIVE_SYNC_POLL_INTERVAL_SECS`, `DRIVE_SYNC_CONFLICT_STRATEGY`).
- **US-18 — Drive sync status**: Complete. `GET /api/drive/status` (auth-guarded) backed by `StatusProvider` interface on `SyncManager`; `DriveStatus` struct with `*string` nullables; `internal/drivesync/handler.go` nil-safe handler; `DriveSyncStatus` React component in header — amber/red/green pill with relative timestamp, popover with full details; polls every 60 s.
- **US-21 — Download a vault file**: Complete. `contentDisposition` helper (RFC 5987 for non-ASCII/spaces, plain `filename=` for ASCII); `vault.Manager.ServeFile` (path validation via `safePath`, `mime.TypeByExtension` with `application/octet-stream` fallback, streams via `http.ServeContent`); `GET /api/vault/files/` route in `internal/server` behind auth middleware; DOWNLOAD LINKS paragraph added to `systemPromptBase` instructing the LLM to construct `/api/vault/files/` links, offer them proactively for binary files, and use `list_vault`/`search_vault` when path is unknown.
- **US-19 — Back up chat history to Google Drive**: Not started. `BackupManager`: `db.Checkpoint` → `tar.gz` → Drive upload every 30 min; graceful shutdown triggers final backup; `drive_sync:last_pebble_backup` in state.
- **US-20 — Restore chat history from Google Drive**: Not started. `RestoreIfNeeded` runs at startup before `pebble.Open`; downloads and extracts backup if `PEBBLE_PATH` absent/empty; no-op if local DB has data or no Drive backup exists.
