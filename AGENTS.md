...
- **US-17 — Sync Drive changes back to vault**: Not started. Polling goroutine (`changes.list`, 60 s); download path; full bidirectional reconcile; conflict resolution (`last_write_wins` / `keep_both` with 5 s window); Drive deletions propagate locally.
