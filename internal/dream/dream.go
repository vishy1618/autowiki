...
// consolidate once per night during the configured UTC window.
func NewRunner(vm *vault.Manager, consolidate ConsolidateFn, startHourUTC, endHourUTC int) *Runner {
	return &Runner{vault: vm, consolidate: consolidate, startHourUTC: startHourUTC, endHourUTC: endHourUTC}
}

// NextFireTime returns a random time in the [startHourUTC, endHourUTC) UTC
// window that is strictly after now.
func NextFireTime(now time.Time, startHourUTC, endHourUTC int) time.Time {
	nowUTC := now.UTC()
	windowSec := (endHourUTC - startHourUTC) * 3600
	offsetSec := rand.Intn(windowSec)
	candidate := time.Date(
		nowUTC.Year(), nowUTC.Month(), nowUTC.Day(),
		startHourUTC, 0, 0, 0, time.UTC,
...
// todayEntry returns true if log.md contains a dated entry for today (UTC).
...
// PostRunSchedulingTime returns endHourUTC:00 UTC of the same day as now,
// which is at the edge of the configured window, guaranteeing NextFireTime picks tomorrow.
func PostRunSchedulingTime(now time.Time, endHourUTC int) time.Time {
	n := now.UTC()
	return time.Date(n.Year(), n.Month(), n.Day(), endHourUTC, 0, 0, 0, time.UTC)
}
...
func (r *Runner) Start(ctx context.Context) {
	now := time.Now()
	for {
		next := NextFireTime(now, r.startHourUTC, r.endHourUTC)
		select {
		case <-ctx.Done():
			return
		...
		r.RunOnce(ctx)
		now = PostRunSchedulingTime(time.Now(), r.endHourUTC)
	}
}
